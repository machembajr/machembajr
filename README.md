# 👋 Hi, I'm Fredrick Machemba
### 📊 Data Analyst | Supply Chain, Operations & Business Intelligence

Turning raw data into clear, actionable insights — with a focus on **supply chain & operational intelligence, healthcare and development analytics, and business performance across African and emerging markets**. I help organizations make smarter decisions through structured analysis, clean reporting, and well-designed dashboards.

📍 Based in Tanzania | Open to remote and hybrid opportunities

---

## 🎯 What I Do

**📦 Supply Chain & Operational Intelligence**
- Supply chain analytics — optimization, risk analysis, and logistics insights
- Operational intelligence — process efficiency, throughput, and bottleneck analysis
- Demand forecasting and inventory optimization

**🏥 Healthcare & Development Analytics**
- Healthcare accessibility, affordability, and cost insights
- Development indicators and program impact analysis
- Public health data structuring and reporting

**💼 Business & Commercial Analytics**
- 📈 **Sales performance analysis** — pipeline, conversion, and revenue trends
- 👥 **Customer segmentation** — behavioral, RFM, and demographic clustering
- 🛒 **E-commerce analytics** — funnel, retention, and product performance
- 📑 **Business performance reporting** — KPIs, scorecards, and executive dashboards

**🧑‍💼 People & Workforce Analytics**
- **HR attrition analytics** — turnover drivers, retention modeling, and workforce trends

**💱 Economic & Market Analysis**
- FX, inflation, and emerging market trends
- Cross-market comparisons and macro context for business decisions

**🎨 Visualization & Decision Support**
- Dashboard development in Power BI & Tableau
- Translating complex datasets into clear visual stories
- Data-driven recommendations for business teams

---

## 🛠️ Tech Stack
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white)
![Power BI](https://img.shields.io/badge/Power%20BI-F2C811?style=for-the-badge&logo=power-bi&logoColor=black)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=for-the-badge&logo=tableau&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Airtable](https://img.shields.io/badge/Airtable-18BFFF?style=for-the-badge&logo=airtable&logoColor=white)

---

## 🔧 My Analytics Workflow

| Stage | What I Do |
|-------|-----------|
| 📥 **Prepare** | Data collection, structuring, and validation |
| 🧹 **Clean** | Handling missing values, duplicates, and standardization in SQL & Excel |
| 📊 **Analyze** | Exploratory analysis, KPI tracking, segmentation, and trend identification |
| 🎨 **Visualize** | Building clear, interactive dashboards in Power BI & Tableau |
| 💡 **Deliver** | Translating findings into business-ready recommendations |

---

## 📂 Focus Areas in My Portfolio

- 📦 Supply Chain & Operational Intelligence
- 🏥 Healthcare & Development Analytics
- 📈 Sales Performance Analysis
- 👥 Customer Segmentation
- 🧑‍💼 HR Attrition Analytics
- 🛒 E-Commerce Analytics
- 📑 Business Performance Reporting
- 💱 Economic & Market Analysis

---

## 📈 Currently Learning

- 🐍 **Python** for data analysis (pandas, numpy, matplotlib)
- 🗂️ **Airtable** for relational data management and workflow automation
- 📦 **Supply Chain Analytics** — advanced logistics, demand forecasting, and inventory optimization
- 📊 Advanced **SQL** and dashboard storytelling in Power BI & Tableau

---

## 💼 What I Bring to a Team

- A structured, detail-oriented approach to messy real-world data
- Clear communication — translating analysis into business language
- Focus on emerging market context (Tanzania, East Africa, African economies)
- Commitment to clean dashboards that decision-makers actually use

---
---

## 📊 My GitHub Stats

<div align="center">


---
## 📬 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/machembajr/)
[![X](https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/Machembajr)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/machembajr)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:machemba.jnr@gmail.com)

---

> 💡 **Open to opportunities:** Data Analyst | Business Analyst | Supply Chain Analyst | BI Developer roles — remote, hybrid, or based in East Africa.
>
> 📁 **See my work below** in pinned repositories.
